...
export default class HistogramPanel extends Panel {
  ...
  prepareGroup(group) {
    ...
    let values = this.att.var(this.varname);
    if (group.name != Symbol.all) {
      values = values.filter((_, i) => this._groupBy[i] == group.name);
    }
    const hist = this.hist(values);
    switch (mode) {
    case 'table':
      return hist;
    case 'plot':
  ...
  updateGroup(group) {
    ...
    case 'table':
      const column = [
          this.doc.createElem('th')
            .css({color: group.color})
            .setElem(group.name == Symbol.all ? 'All' : group.name),
        ];
