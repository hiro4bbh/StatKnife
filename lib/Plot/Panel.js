...
export default class Panel extends StatKnifePanel {
  ...
  constructor(...args) {
    ...
    this._margin = {top: 16, right: 16, bottom: 48, left: 48};
  ...
  lines(data, options) {
    options = StatKnife.setDefault(options, {color: '#000000', fillColor: 'none'});
    if (!this.svg) {
    ...
    return this.svg.append('g')
      .datum(data)
      .append('path')
      .attr('d', d3.line().x((d) => this._xScale(d[0])).y((d) => this._yScale(d[1])))
      .attr('fill', options.fillColor)
      .attr('stroke', options.color)
      .attr('stroke-width', 2.0);
  }
  points(data, options) {
    options = StatKnife.setDefault(options, {fillColor: '#000000', r: 4.0});
    if (!this.svg) {
