...
export default class Panel extends StatKnifePanel {
  static onimported(StatKnife) {
    return new Promise(async (resolve) => {
      await StatKnife.importFromSystemPath('Plot/PlotPanel');
      await StatKnife.importFromSystemPath('Plot/HistogramPanel');
      resolve();
    });
  }
  constructor(...args) {
  ...
  save() {
    if (this.container.children.length == 1 && this.container.children[0].tagName == 'svg') {
      ...
      const blob = new Blob([(new XMLSerializer()).serializeToString(svg)], {type: 'text/xml'});
      const filename = 'panel.svg';
