...
export class ArraySummaryPanel extends Panel {
  constructor(data, ...args) {
    ...
    for (let value of Object.values(this.table)) {
      ...
      valrow.newChild('td').attrs({class: 'center'}).setElem(value);
    }
  }
  get typename() {
...
export class Attachment {
  ...
  hasVar(name) {
    return this.env.colnames.find((colname) => colname == name);
  }
  typeof(name) {
    return this.env.typeof(name);
  }
  var(name) {
    return this.env.column(name);
  }
}
