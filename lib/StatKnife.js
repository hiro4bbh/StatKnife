...
export class Panel {
  constructor(name, option) {
    ...
    this.buttonSave = this.controllers.newChild('button').setElem('Save');
  ...
  save() {
    let blob, filename;
    if (this.container.children.length == 1 && this.container.children[0].tagName == 'svg') {
      console.log(`save panel #${this.id()}(${this.name()}) as SVG`);
      const svg = this.container.children[0];
      blob = new Blob([(new XMLSerializer()).serializeToString(svg)], {type: 'text/xml'});
      filename = 'panel.svg';
    } else {
      ...
      blob = new Blob([htmlHeader+this.container.innerHTML+htmlFooter], {type: 'text/html'});
      filename = 'panel.html';
    }
    const url = window.URL.createObjectURL(blob);
    this.doc.newElem('a').attrs({href: url, download: filename}).click();
    return this;
  }
}
